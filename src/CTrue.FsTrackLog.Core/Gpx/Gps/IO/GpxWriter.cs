...
namespace SpatialLite.Gps.IO {
    ...
    public class GpxWriter : IDisposable, IGpxWriter {
        ...
        public void WriteTrackStart()
        {
            _xmlWriter.WriteStartElement("trk");
            _xmlWriter.WriteStartElement("trkseg");
        }

        public void WriteTrackPoint(GpxPoint trackPoint)
        {
            WritePoint(trackPoint, "trkpt");
        }

        public void WriteTrackEnd(GpxTrackMetadata trackMetadata = null)
        {
            _xmlWriter.WriteEndElement(); // trkseg

            if (this.Settings.WriteMetadata && trackMetadata != null)
            {
                this.WriteTrackMetadata(trackMetadata);
            }
